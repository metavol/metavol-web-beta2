...
それでもモダリティが間違って表示される場合の**最終手段**として、左サイドバーのシリーズカードの
**「…」→ Change modality → Set as PT / CT / MR** で手動指定できる。手動指定したシリーズは
チップが `PT*` のように * 付き・点線枠になる。**Auto (from file)** で自動判定に戻せる。
手動指定はメモリ上だけで、ファイルは書き換えない（再読み込みすると自動判定に戻る）。
PET と CT を切り替えると画像を作り直すため、そのシリーズ上のマスクは消える（確認が出る）。
