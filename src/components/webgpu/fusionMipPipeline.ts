// Fusion MIP compute pipeline + 1-call render API。

import { getGpuDevice } from './gpuContext';
import { getVolumeTexture } from './volumeCache';
import { FUSION_MIP_SHADER_WGSL } from './fusionMipShader';
...
const UNIFORM_SIZE = 176;   // 11 vec4 × 16
...
const ensurePipeline = async (): Promise<PipelineCache | null> => {
    ...
    const module = device.createShaderModule({ code: FUSION_MIP_SHADER_WGSL });
    const bindLayout = getFusionMipBindGroupLayout(device);
...
export interface GpuFusionMipParams {
    ...
    angle: number;
    overlayBlend: number;       // 0..1
    targetCanvas: HTMLCanvasElement;
}

export const gpuRenderFusionMip = async (
    p: GpuFusionMipParams,
